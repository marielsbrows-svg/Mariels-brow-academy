...
export const SlideViewer = ({ lessonId, onComplete }: SlideViewerProps) => {
  ...
  useEffect(() => {
    setLoading(true);
    setCurrentSlide(1);
    supabase.from('lesson_slides').select('*')
  ...
  const currentImage = slides.find(s => s.slide_number === currentSlide)?.image_url;

  const goNext = () => {
    if (currentSlide < numPages) setCurrentSlide(n => n + 1);
    else onComplete?.();
  };
      ...
      {!isFullscreen && (
        <div className="border border-white/08 overflow-hidden">
          ...
          <div className="relative bg-black w-full">
            {currentImage && (
              <img src={currentImage} alt={`Slide ${currentSlide}`} className="w-full block" />
            )}
            <div className="absolute top-2 right-2 bg-black/50 px-2 py-1">
              <span className="text-[0.5rem] text-white/70 tracking-widest">{currentSlide} / {numPages}</span>
            </div>
            <AudioBar />
          </div>
          <NavBar />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="flex-1 overflow-auto flex items-center justify-center p-4">
              <div className="relative w-full max-w-5xl">
                {currentImage && <img src={currentImage} alt={`Slide ${currentSlide}`} className="w-full block" />}
                <AudioBar />
              </div>
            </div>
            <NavBar />
